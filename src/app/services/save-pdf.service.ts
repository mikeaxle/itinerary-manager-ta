...
export class SavePdfService {
  ...
  constructor(private data: DataService, private http: HttpClient, private countryService: CountryService) {
  ...
  savePDF(key: any, type: string, usedDays: any) {
    // assign used days
    ...
    // get itinerary
    this.data.getSingleItem(key, `itineraries/${this.currentCompany}`)
      .valueChanges()
      .subscribe((res) => {
      this.itinerary = res;

      // assign discount
      if (this.itinerary.discount === undefined) {
        this.itinerary.discount = 0;
      }
      ...
      // assign deposit
      if (this.itinerary.deposit === undefined) {
        this.itinerary.deposit = 0;
      }

      // assign total
      if (this.itinerary.total === undefined) {
      ...
      // @ts-ignore
      this.totalDays = (Date.parse(res.enddate) - Date.parse(res.startdate)) / 86400000;
      this.totalDays++;

    });

    // get days
    this.data.af.list(`days/${key}`)
      .valueChanges()
      .subscribe((res) => {
      // assign day variable
      this.days = res;
      ...
      this.days.forEach(day => {
        ...
        if (day.accommodation !== undefined) {
          ...
          day.accommodation.forEach((a) => {
            ...
            // replace long description with original
    ...
    // get comments, payments and phone numbers associated with itinerary
    this.data.af.list(`comments/${key}`)
      .valueChanges()
      .subscribe((res) => this.comments = res);

    // get phones numbers
    this.data.af.list(`phone_numbers/${key}`)
      .valueChanges()
      .subscribe((res) => this.phone_numbers = res);

    // get payments
    this.data.af.list(`payments/${key}`)
      .valueChanges()
      .subscribe((res) => {
    ...
    <div class="container">
      ...
      <ul class="sidebar">
        ...
        <li>
          ...
          <p class="field">${this.itinerary.adults}</p>
        </li>
        ${this.getChildren()}
        <li>
    ...
    <div class="itenerary-quote">
      ...
      <table class="quoting" cellspacing="0">
        ...
        <tr class="total-price">
          ...
          <td class="currency">USD</td>
          <td class="amount">${this.itinerary.total}</td>
        </tr>
...
if (lastIteneraryItemsHeight + quotHeight > quoteThreshold) {
  ...
  getPayments() {
    ...
    this.payments.forEach((p) => {
      ...
      <td class="amount space">-${p.amount}</td>
    </tr>
      `;
    });
    ...
    accommodation.forEach(a => {
      ...
      <div class="wrapper">
        ...
        <div class="accom-desc">
          ...
          <p class="desc">${a.long_description}</p>
        </div>
      </div>`;
  ...
  getChildren() {
    ...
    if (this.itinerary.children_ages !== undefined && this.itinerary.children > 0) {
      html += `
      <li>
        <p class="title">Children</p>
        <p class="field">${this.itinerary.children}</p>
      </li>
      <li>
        <p class="title ages">Ages at time of Travel</p>
        <p class="field">${this.itinerary.children_ages}</p>
      </li>`;
    }
  ...
  getHeaderLogo() {
    switch (this.currentCompany) {
      case 'True Africa':
        this.headerLogoSrc = 'pdf-img/logo-icon@2x.png';
  ...
  showDiscountAndDeposit() {
    ...
    if (this.itinerary.discount > 0) {
      ...
      _html += `
          ...
          <td class="currency">USD</td>
          <td class="amount">${this.itinerary.discount}</td>
        </tr>`;
    }
    ...
    if (this.itinerary.deposit > 0) {
      _html += `<tr class="deposit" >
          ...
          <td class="currency">USD</td>
          <td class="amount">${this.itinerary.deposit}</td>
        </tr>
        `;
    }
